...
class PositionCalculator {
  ...
  int findNearestThumbIndex(
    double normalizedPosition,
    List<double> normalizedPositions,
  ) {
    int nearestIndex = 0;
    double minDistance = double.infinity;

    for (int i = 0; i < normalizedPositions.length; i++) {
      final double distance = (normalizedPosition - normalizedPositions[i])
          .abs();
  ...
  double calculateUpperBound(int thumbIndex, List<double> normalizedPositions) {
    return thumbIndex == normalizedPositions.length - 1
        ? 1.0
        : normalizedPositions[thumbIndex + 1];
  }
}
